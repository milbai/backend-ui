...
const DeviceInstancePage: React.FC<Props> = props => {
  ...
  const columns: ColumnProps<DeviceInstance>[] = [
    ...
    },
    {
      ...
      //dataIndex: 'state',
      width: '90px',
      render: record => (record.productId != 'videoMonitor' && record.productId != 'M401A' && record.state) ? <Badge status={statusMap.get(record.state.value)} text={record.state.text} /> : '',
      filters: [
        {
          text: '未启用',
          value: 'notActive',
    ...
    },
    {
      ...
      render: (record: any) => (
        <Fragment>
          <a
            onClick={() => {
              router.push(`/device/alarm-device/save/${record.id}`);
            }}
          >
            设置
          </a>
